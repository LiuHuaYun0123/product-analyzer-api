...
file_path = f"C:/Users/liu.huayun/Desktop/text/products_v1.jsonl"
...
try:
    ...
    if suspicious_values:
        ...
        if os.path.exists(suspicious_csv_path):
            # 如果文件存在，读取 CSV 文件，获取已有的字段值
            existing_df = pd.read_csv(suspicious_csv_path, encoding='utf-8-sig')

            # 检查 CSV 是否为空
            if existing_df.empty:
                existing_values = set()
            else:
                # 检查 'Suspicious Values' 列是否存在
                if "Suspicious Values" in existing_df.columns:
                    existing_values = set(existing_df["Suspicious Values"].values)
        ...
        if new_values:
            # 如果有新的不重复的字段值，写入 CSV
            new_df = pd.DataFrame(list(new_values), columns=["Suspicious Values"])

            # 如果文件不存在，创建并写入列标题；如果文件存在，追加数据但不重复列标题
            new_df.to_csv(suspicious_csv_path, index=False, encoding='utf-8-sig', mode='a', header=not os.path.exists(suspicious_csv_path))
    ...
    if documents:
        ...
        response = wr.opensearch.index_documents(
            ...
            documents=documents,   # <--- 传递解析后的字典列表
            use_threads=True,
            index=index_name,
            bulk_size=200,
            refresh=False
