...
output_jsonl_path = 'products_data_name.jsonl'  # 输出 JSONL 文件路径
# --- 配置结束 ---
...
columns_needed = [
    "product_management_code",
    "entry_category_name", "category_name", "major_classification_name", "medium_classification_name","manufacturer_name", 
    "manufacturer_name_kana","model_number","common_name",
    "product_name", "material_1", "material_2", "color_1", "color_2","feature_1_design","feature_3_waterproof",
...
# --- 新功能 2：创建 priority_high，拼接 columns_needed 中字段值 ---
# 新增优先字段列表
priority_fields = [
...
# 修改 build_priority_high 函数，使用 priority_fields
...
# 添加 popularity 字段，1 到 100 之间的随机浮点数
df["popularity"] = df.apply(lambda _: round(random.uniform(1, 100), 2), axis=1)

# 添加 priority_high popularity 字段到最终输出字段中
